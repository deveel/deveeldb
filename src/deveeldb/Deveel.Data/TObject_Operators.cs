	...
	public sealed partial class TObject {
		...
		public TObject Add(TObject val) {
			if (TType is TDateType) {
				...
				throw new ArgumentException("Type TDateType only supports addition of TIntervalType instances.");
			}
			...
			throw new InvalidOperationException(String.Format("Type {0} does not support value addition.", TType));
		}
		...
		public TObject Subtract(TObject val) {
			if (TType is TDateType) {
				...
				throw new ArgumentException("Type TDateType only supports substraction of TIntervalType and TDateType instances.");
			}
			if (TType is TIntervalType) {
				Interval v1 = ToInterval();
				if (val.TType is TIntervalType) {
			...
			throw new InvalidOperationException(String.Format("Type {0} does not support value substraction.", TType));
		}
