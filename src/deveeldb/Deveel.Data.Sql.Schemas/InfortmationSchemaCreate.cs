...
namespace Deveel.Data.Sql.Schemas {
	class InfortmationSchemaCreate : IDatabaseCreateCallback {
		...
		private static void GrantToPublic(IQuery query) {
			query.Access()
				.GrantOn(DbObjectType.View, InformationSchema.ThisUserSimpleGrantViewName, User.PublicName, Privileges.TableRead);
			query.Access()
				.GrantOn(DbObjectType.View, InformationSchema.ThisUserGrantViewName, User.PublicName, Privileges.TableRead);
			query.Access()
				.GrantOn(DbObjectType.View, InformationSchema.ThisUserSchemaInfoViewName, User.PublicName, Privileges.TableRead);
			query.Access()
				.GrantOn(DbObjectType.View, InformationSchema.ThisUserTableInfoViewName, User.PublicName, Privileges.TableRead);
			query.Access()
				.GrantOn(DbObjectType.View, InformationSchema.ThisUserTableColumnsViewName, User.PublicName, Privileges.TableRead);
