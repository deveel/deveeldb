...
namespace Deveel.Data.Routines {
	/// <summary>
	/// An object that provides instances of <see cref="IFunction"/>
	/// already defined.
	/// </summary>
	public abstract class FunctionFactory : IRoutineResolver {
		...
		/// <summary>
		/// Default contructor of the factory.
		/// </summary>
		...
		/// <summary>
		/// Gets a special expression that represents the <c>glob</c> argument (*).
		/// </summary>
		public static SqlExpression GlobExpression { get; private set; }

		public static SqlExpression[] GlobList { get; private set; }

		public ObjectName DefaultSchemaName { get; private set; }

		/// <summary>
		/// Creates an alias for the given simple function name
		/// </summary>
		...
		private InvokeRequest UnAlias(InvokeRequest request) {
			var name = request.RoutineName.Name;

			string alias;
			if (!aliases.TryGetValue(name, out alias))
				return request;

			return new InvokeRequest(new ObjectName(request.RoutineName.Parent, alias), request.Arguments);
		}
		...
		public IRoutine ResolveRoutine(InvokeRequest request, IQueryContext context) {
			request = UnAlias(request);

			object arg = null;
			FunctionInfo info = null;
			foreach (var mapping in functionTypeMapping) {
				if (mapping.Key.MatchesInvoke(request, context)) {
					if (arg != null)
						throw new AmbiguousMatchException(String.Format("More than one overload of '{0}' matches the given call.", request.RoutineName));
