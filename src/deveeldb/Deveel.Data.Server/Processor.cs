...
namespace Deveel.Data.Server {
	...
	public abstract class Processor {
		...
		protected Processor(DbController controller, string host_string) {
		...
		protected byte[] ProcessCommand(byte[] command) {
			if (state == ClientConnectionState.Closed) {
				...
				*/
				...
				//
				//      return null;
			} else if (state == ClientConnectionState.NotAuthenticated) {
				...
				return null;
			} else if (state == ClientConnectionState.Processing) {
				// Process the query
