...
namespace Deveel.Data.Index {
	public sealed class IndexRangeSet {
		...
		private static IndexRange IntersectOn(IndexRange range, SelectCondition condition, DataObject value, bool nullCheck) {
			var start = range.StartValue;
			var startPosition = range.StartOffset;
			var end = range.EndValue;
			var endPosition = range.EndOffset;

			bool inclusive = condition.ConditionType == ConditionType.Is ||
			                 condition.ConditionType == ConditionType.Equal ||
			                 condition.ConditionType == ConditionType.GreaterOrEqualThan ||
			                 condition.ConditionType == ConditionType.SmallerOrEqualThan;

			if (condition.ConditionType == ConditionType.Is ||
			    condition.ConditionType == ConditionType.Equal ||
			...
			if (condition.ConditionType == ConditionType.Is ||
			    condition.ConditionType == ConditionType.Equal ||
			    condition.ConditionType == ConditionType.SmallerThan ||
			    condition.ConditionType == ConditionType.SmallerOrEqualThan) {
				// With this operator, NULL values must return null.
				if (nullCheck && value.IsNull) {
					return IndexRange.Null;
				...
				if (nullCheck && start == IndexRange.FirstInSet) {
					start = DataObject.Null();
					startPosition = RangeFieldOffset.AfterLastValue;
				}

				if (end == IndexRange.LastInSet) {
					end = value;
			...
			if (start != IndexRange.FirstInSet && end != IndexRange.LastInSet) {
		...
		private static bool IntersectedBy(IndexRange range1, IndexRange range2) {
			...
			var startCell1 = start1 == IndexRange.FirstInSet ? null : start1;
			var endCell1 = end1 == IndexRange.LastInSet ? null : end1;
			var startCell2 = start2 == IndexRange.FirstInSet ? null : start2;
			var endCell2 = end2 == IndexRange.LastInSet ? null : end2;
		...
		private static IndexRange ChangeRangeSizeToEncompass(IndexRange range1, IndexRange range2) {
			...
			if (start1 != IndexRange.FirstInSet) {
				if (start2 != IndexRange.FirstInSet) {
					var cell = start1;
			...
			if (end1 != IndexRange.LastInSet) {
				if (end2 != IndexRange.LastInSet) {
					var cell = end1;
		...
		public IndexRangeSet Intersect(SelectCondition condition, DataObject value) {
			lock (this) {
				int sz = ranges.Count;
				var list = ranges.GetRange(0, sz);

				if (condition.ConditionType == ConditionType.NotEqual ||
				    condition.ConditionType == ConditionType.IsNot) {
					bool nullCheck = condition.ConditionType == ConditionType.NotEqual;
					int j = 0;
					while (j < sz) {
						var range = list[j];
						var leftRange = IntersectOn(range, new SelectCondition(ConditionType.SmallerThan), value, nullCheck);
						var rightRange = IntersectOn(range, new SelectCondition(ConditionType.GreaterThan), value, nullCheck);
						list.RemoveAt(j);
					...
					bool nullCheck = condition.ConditionType != ConditionType.Is;
					int j = 0;
					while (j < sz) {
						var range = list[j];
						range = IntersectOn(range, condition, value, nullCheck);
				...
				while (n < inSz) {
					...
					if (start == IndexRange.FirstInSet) {
						...
						while (j < i.Count) {
							...
							if (curStart != IndexRange.FirstInSet) {
