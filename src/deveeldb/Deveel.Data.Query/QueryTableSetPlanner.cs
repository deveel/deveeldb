...
namespace Deveel.Data.Query {
	...
	internal sealed class QueryTableSetPlanner {
		...
		public void AddTableSource(IQueryPlanNode plan, IFromTableSource fromTable) {
			VariableName[] allCols = fromTable.AllColumns;
		...
		private static PlanTableSource ConcatTableSources(PlanTableSource left, PlanTableSource right, IQueryPlanNode plan) {
			...
			var newVarList = new VariableName[left.VariableNames.Length + right.VariableNames.Length];
			int i = 0;
			foreach (VariableName v in left.VariableNames) {
				newVarList[i] = v;
				++i;
			}
			foreach (VariableName v in right.VariableNames) {
				newVarList[i] = v;
				++i;
			}
		...
		/// </remarks>
		/// <returns></returns>
		private PlanTableSource JoinAllPlansWithVariables(IEnumerable<VariableName> allVars) {
		...
		private static void AddSingleVarPlanTo(IList<SingleVarPlan> list, PlanTableSource table, VariableName variable, VariableName singleVar, Expression[] expParts, Operator op) {
			var exp = new Expression(expParts[0], op, expParts[1]);
		...
		private void PlanForExpressionList(IEnumerable<Expression> andList) {
			...
			foreach (Expression andexp in andList) {
				...
				// '[expression] = true'
				if (!(lob is Operator) || ((Operator)lob).IsMathematical) {
		...
		public IQueryPlanNode PlanSearchExpression(SearchExpression searchExpression) {
			...
			return LogicalEvaluate(searchExpression.FromExpression);
		}
