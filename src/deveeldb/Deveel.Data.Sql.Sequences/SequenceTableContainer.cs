...
namespace Deveel.Data.Sql.Sequences {
	class SequenceTableContainer : ITableContainer {
		...
		public int FindByName(ObjectName tableName) {
			return Manager.Offset(tableName);
		}

		public ObjectName GetTableName(int offset) {
			var name = Manager.NameAt(offset);
			if (name == null)
		...
		public ITable GetTable(int offset) {
			var tableName = Manager.NameAt(offset);
			if (tableName == null)
				throw new Exception();

			// Generate the DataTableInfo
			var tableInfo = CreateTableInfo(tableName.Parent, tableName.Name);
			...
			var startValue = Field.Number(sequence.SequenceInfo.StartValue);
			var incrementBy = Field.Number(sequence.SequenceInfo.Increment);
			var minValue = Field.Number(sequence.SequenceInfo.MinValue);
			var maxValue = Field.Number(sequence.SequenceInfo.MaxValue);
			var cache = Field.BigInt(sequence.SequenceInfo.Cache);
			var cycle = Field.Boolean(sequence.SequenceInfo.Cycle);
