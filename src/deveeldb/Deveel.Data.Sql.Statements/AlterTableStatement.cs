...
namespace Deveel.Data.Sql.Statements {
	...
	public sealed class AlterTableStatement : SqlStatement {
		...
		protected override void ExecuteStatement(ExecutionContext context) {
			if (!context.Request.Query.Session.Access.UserCanAlterTable(TableName))
			...
			for (int n = 0; n < tableInfo.ColumnCount; ++n) {
				...
				if (Action.ActionType == AlterTableActionType.SetDefault &&
					...
					var refs = context.Request.Query.Session.Access.QueryTableImportedForeignKeys(TableName);
					foreach (var reference in refs) {
						CheckColumnConstraint(columnName, reference.ForeignColumnNames, reference.ForeignTable, reference.ConstraintName);
					}

					// Or from it
					refs = context.Request.Query.Session.Access.QueryTableForeignKeys(TableName);
					foreach (var reference in refs) {
					...
					var primaryKey = context.Request.Query.Session.Access.QueryTablePrimaryKey(TableName);
					if (primaryKey != null)
					...
					var uniques = context.Request.Query.Session.Access.QueryTableUniqueKeys(TableName);
			...
			if (Action.ActionType == AlterTableActionType.DropConstraint) {
				...
				int dropCount = context.Request.Query.Session.Access.DropTableConstraint(TableName, constraintName);
				if (dropCount == 0)
				...
				if (!context.Request.Query.Session.Access.DropTablePrimaryKey(TableName, null))
			...
			if (Action.ActionType == AlterTableActionType.AddConstraint) {
				...
				if (foreignConstraint) {
					refTname = context.Request.Query.Session.Access.ResolveTableName(constraint.ReferenceTable);
				}
				...
				context.Request.Query.Session.Access.AddConstraint(TableName, newConstraint);
			}
			...
			if (tableAltered) {
				...
				context.Request.Query.Session.Access.AlterTable(newTableInfo);
			} else {
				...
				context.Request.Query.Session.Access.CheckConstraintViolations(TableName);
			}
		}
