	...
	internal sealed partial class Planner {
		...
		abstract class ExpressionPlan : IComparable {
			...
			public int CompareTo(object ob) {
				ExpressionPlan destPlan = (ExpressionPlan)ob;
				float dest_val = destPlan.optimizableValue;
				return optimizableValue > dest_val ? 1 : (optimizableValue < dest_val ? -1 : 0);
			}
		}
		...
		private sealed class QueryTableSetPlanner {
			...
			public void AddTableSource(IQueryPlanNode plan, IFromTableSource fromDef) {
				...
				string[] unique_names = new string[] { fromDef.UniqueName };
				AddPlanTableSource(new PlanTableSource(plan, allCols, unique_names));
			}
			...
			private int IndexOfPlanTableSource(PlanTableSource source) {
				...
				*/
				...
				return tableList.FindIndex(delegate(PlanTableSource s) { return s == source; });
			}
			...
			private static PlanTableSource ConcatTableSources(PlanTableSource left, PlanTableSource right, IQueryPlanNode plan) {
				...
				VariableName[] newVarList = new VariableName[left.Variables.Length + right.Variables.Length];
				int i = 0;
				for (int n = 0; n < left.Variables.Length; ++n) {
					newVarList[i] = left.Variables[n];
					++i;
				}
				for (int n = 0; n < right.Variables.Length; ++n) {
					newVarList[i] = right.Variables[n];
					++i;
				}
			...
			/// </remarks>
			/// <returns></returns>
			private PlanTableSource FindCommonTableSource(IList<VariableName> varList) {
				if (varList.Count == 0)
					return null;

				PlanTableSource plan = FindTableSource(varList[0]);
				int i = 1;
				int sz = varList.Count;
				while (i < sz) {
					PlanTableSource p2 = FindTableSource(varList[i]);
					if (plan != p2)
						return null;
			...
			private PlanTableSource NaturallyJoinPlans(PlanTableSource plan1, PlanTableSource plan2) {
				...
				if (plan1.RightPlan == plan2) {
					...
					if ((plan1.LeftPlan != null && plan2.LeftPlan != null) ||
						...
						throw new Exception("Assertion failed - plans can not be naturally join because " +
						                    "the left/right join plans clash.");
					}

					// Else we must assume a non-dependant join (not an outer join).
					// Perform a natural join
					IQueryPlanNode node1 = new QueryPlan.NaturalJoinNode(plan1.Plan, plan2.Plan);
				...
				if (joinType == JoinType.Left) {
					// Mark the left plan
					leftPlan.UpdatePlan(new QueryPlan.MarkerNode(leftPlan.Plan, "OUTER_JOIN"));
					outerJoin = true;
				} else if (joinType == JoinType.Right) {
					// Mark the right plan
					rightPlan.UpdatePlan(new QueryPlan.MarkerNode(rightPlan.Plan, "OUTER_JOIN"));
					outerJoin = true;
			...
			private sealed class SingleVarPlan {
				public PlanTableSource table_source;
				public VariableName single_var;
			...
			/// <param name="op"></param>
			private static void AddSingleVarPlanTo(List<SingleVarPlan> list, PlanTableSource table, VariableName variable, VariableName singleVar, Expression[] exp_parts, Operator op) {
				Expression exp = new Expression(exp_parts[0], op, exp_parts[1]);
				// Is this source in the list already?
				foreach (SingleVarPlan varPlan in list) {
					if (varPlan.table_source == table &&
						(variable == null || varPlan.variable.Equals(variable))) {
						// Append to end of current expression
						varPlan.variable = variable;
						varPlan.expression = new Expression(varPlan.expression, Operator.And, exp);
						return;
					}
				}
			...
			private class ConstantExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					...
					foreach (PlanTableSource plan in qtsp.tableList) {
						plan.UpdatePlan(new QueryPlan.ConstantSelectNode(plan.Plan, expression));
					}
				}
			}
			...
			private class SimpleSelectExpressionPlan : ExpressionPlan {
				...
				public SimpleSelectExpressionPlan(QueryTableSetPlanner qtsp, VariableName v, Operator op, Expression e) {
					this.qtsp = qtsp;
					single_var = v;
					this.op = op;
					expression = e;
				...
				public override void AddToPlanTree() {
					...
					PlanTableSource tableSource = qtsp.FindTableSource(single_var);
					tableSource.UpdatePlan(new QueryPlan.SimpleSelectNode(tableSource.Plan, single_var, op, expression));
				}
			}
			...
			private class SimpleSingleExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					// Find the table source for this variable
					PlanTableSource tableSource = qtsp.FindTableSource(single_var);
					tableSource.UpdatePlan(new QueryPlan.RangeSelectNode(tableSource.Plan, expression));
				}
			}
			...
			private class ComplexSingleExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					// Find the table source for this variable
					PlanTableSource tableSource = qtsp.FindTableSource(single_var);
					tableSource.UpdatePlan(new QueryPlan.ExhaustiveSelectNode(tableSource.Plan, expression));
				}
			}
			...
			private class SimplePatternExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					// Find the table source for this variable
					PlanTableSource tableSource = qtsp.FindTableSource(single_var);
					tableSource.UpdatePlan(new QueryPlan.SimplePatternSelectNode(tableSource.Plan, expression));
				}
			}
			...
			private class ExhaustiveSubQueryExpressionPlan : ExpressionPlan {
				private readonly QueryTableSetPlanner qtsp;
				private readonly IList<VariableName> all_vars;
				private readonly Expression expression;
				public ExhaustiveSubQueryExpressionPlan(QueryTableSetPlanner qtsp, IList<VariableName> vars, Expression e) {
					this.qtsp = qtsp;
					this.all_vars = vars;
					this.expression = e;
				}
				public override void AddToPlanTree() {
					...
					// Update the plan
					table_source.UpdatePlan(new QueryPlan.ExhaustiveSelectNode(table_source.Plan, expression));
			...
			private class SimpleSubQueryExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					...
					// Update the plan
					tableSource.UpdatePlan(new QueryPlan.NonCorrelatedAnyAllNode(leftPlan, rightPlan, new VariableName[] {leftVar}, op));
				}
			}
			...
			private class ExhaustiveJoinExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					...
					allPlan.UpdatePlan(new QueryPlan.ExhaustiveSelectNode(allPlan.Plan, expression));
				}
			}
			...
			private class StandardJoinExpressionPlan : ExpressionPlan {
				...
				public override void AddToPlanTree() {
					...
					VariableName lhsV = exps[0].VariableName;
					VariableName rhsV = exps[1].VariableName;
					...
					if (lhsPlan != rhsPlan) {
						...
						if (lhsV != null || rhsV != null) {
							// If rhsV is a single variable and lhsV is not then we must
							// reverse the expression.
							QueryPlan.JoinNode joinNode;
							if (lhsV == null) {
								// Reverse the expressions and the operator
								joinNode = new QueryPlan.JoinNode(rhsPlan.Plan, lhsPlan.Plan, rhsV, op.Reverse(), exps[0]);
								qtsp.MergeTables(rhsPlan, lhsPlan, joinNode);
							} else {
								// Otherwise, use it as it is.
								joinNode = new QueryPlan.JoinNode(lhsPlan.Plan, rhsPlan.Plan, lhsV, op, exps[1]);
					...
					IList<VariableName> allVars = expression.AllVariables;
					// Merge it into one plan (possibly performing natural joins).
					PlanTableSource all_plan = qtsp.JoinAllPlansWithVariables(allVars);
					// And perform the exhaustive select,
					all_plan.UpdatePlan(new QueryPlan.ExhaustiveSelectNode(all_plan.Plan, expression));
				}
			}
			...
			private void EvaluateSingles(IList<Expression> singleVars, IList<ExpressionPlan> evaluateOrder) {
				...
				foreach (SingleVarPlan varPlan in simplePlanList) {
					ExpressionPlan expPlan = new SimpleSingleExpressionPlan(this,varPlan.single_var, varPlan.expression);
					expPlan.OptimizableValue = 0.2f;
					evaluateOrder.Add(expPlan);
				}

				foreach (SingleVarPlan varPlan in complexPlanList) {
					ExpressionPlan expPlan = new ComplexSingleExpressionPlan(this, varPlan.single_var, varPlan.expression);
					expPlan.OptimizableValue = 0.8f;
					evaluateOrder.Add(expPlan);
				}
			}
			...
			/// </remarks>
			private void EvaluatePatterns(IList<Expression> patternExprs, IList<ExpressionPlan> evaluateOrder) {
				...
				foreach (Expression expr in patternExprs) {
					...
					} else if (lhsV != null && exps[1].IsConstant) {
						ExpressionPlan exprPlan = new SimplePatternExpressionPlan(this, lhsV, expr);
			...
			/// expression.
			/// </remarks>
			private void EvaluateSubLogic(IList<Expression> sublogic_exprs, IList<ExpressionPlan> evaluate_order) {
				//each_logic_expr:
				foreach (Expression expr in sublogic_exprs) {
					...
					if (lastOp.Is("or")) {
						...
						for (int i = 0; i < sz; ++i) {
							...
							if (leftPlan.IsUpdated || rightPlan.IsUpdated) {
								...
								// the plan for this.
								IQueryPlanNode node = new QueryPlan.LogicalUnionNode(leftPlan.Plan, rightPlan.Plan);
