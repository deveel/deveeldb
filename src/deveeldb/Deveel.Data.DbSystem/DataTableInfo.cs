...
namespace Deveel.Data.DbSystem {
	...
	public sealed class DataTableInfo : ICloneable {
		...
		/// </returns>
		/// <exception cref="ArgumentNullException">If <paramref name="columnName"/> is <b>null</b>
		...
		/// <exception cref="ArgumentNullException">If any column name of the list 
		...
		public DataColumnInfo AddColumn(string name, TType type, bool notNull) {
			DataColumnInfo column = AddColumn(name, type);
			column.IsNotNull = notNull;
			return column;
		}

		public DataColumnInfo AddColumn(string name, TType type) {
			CheckMutable();
		...
		/// <summary>
		/// Gets the <see cref="DataColumnInfo"/> object representing the 
		/// column at the given index.
		/// </summary>
		/// <param name="column">Index of the coulmn to get.</param>
		/// <returns>
		/// Returns a <see cref="DataColumnInfo"/> at the given 
		...
		internal static DataTableInfo Read(BinaryReader input) {
			...
			for (int i = 0; i < size; ++i) {
				DataColumnInfo colInfo = DataColumnInfo.Read(tableInfo, input);
