	...
	public sealed class IndexSetStore : IDisposable {
		...
		public IRowIndexSet GetSnapshotIndex() {
			lock (this) {
				...
				foreach (var indexBlock in snapshotIndexBlocks) {
		...
		class Index : SortedCollection<SqlObject, long>, IRowIndex, IDisposable {
			...
			public IEnumerable<ICollectionBlock<SqlObject, long>> AllBlocks => Blocks.ToArray();

			public IEnumerable<IMappedBlock> DeletedBlocks => deletedBlocks.ToArray();
