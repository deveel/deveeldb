...
namespace Deveel.Data.Sql.Statements {
	public sealed class RevokeRoleStatementTests : IDisposable {
		private string revokedUser;
		private string revokedRole;
		...
		[Theory]
		[InlineData("user2", "role1")]
		public async void UserInAdminRoleRevokesOtherUserRole(string user, string role) {
			var statement = new RevokeRoleStatement(user, role);
		...
		[Theory]
		[InlineData("user2", "role1", "REVOKE role1 FROM user2")]
