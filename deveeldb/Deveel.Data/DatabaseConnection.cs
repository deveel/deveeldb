		...
		*/
		...
		private void FirePendingTriggerEvents() {
			...
			if (sz > 0) {
				...
				database.PostEvent(3, database.CreateEvent(new EventHandler(FirePendingTriggerEventsDel)));
			}

		}

		private void FirePendingTriggerEventsDel(object sender, EventArgs e) {
			lock (trigger_event_buffer) {
				...
				for (int i = 0; i < trigger_event_buffer.Count; i += 2) {
					...
					TriggerEvent evt =
								(TriggerEvent)trigger_event_buffer[i + 1];
					call_back.TriggerNotify(trigger_name, evt.Type,
											evt.Source, evt.Count);
				}
				// Clear the buffer
