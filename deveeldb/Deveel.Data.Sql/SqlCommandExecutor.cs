...
namespace Deveel.Data.Sql {
	///<summary>
	/// An object used to execute SQL commands against a given 
	...
	/// as necessary.
	/// <para>
	/// This object is a convenient way to execute SQL commands.
	/// </para>
	/// </remarks>
	public class SqlCommandExecutor {
		...
		public Table Execute(DatabaseConnection connection, SqlCommand command) {
			...
			IExpressionPreparer preparer = new ExpressionPreparerImpl(command);
		...
		private class ExpressionPreparerImpl : IExpressionPreparer {
			...
			public ExpressionPreparerImpl(SqlCommand command) {
			...
			public Object Prepare(Object element) {
				...
				object value;
				if (command.ParameterStyle == ParameterStyle.Named) {
					string param_name = ps.Name;
					value = command.GetNamedVariable(param_name);
				} else {
					int param_id = ps.Id;
					value = command.Variables[param_id];
				}
