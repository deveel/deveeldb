...
// This class was derived from the GNU Classpath's gnu.math.MPN
...
namespace Deveel.Math {
	class MPN {
		...
		public static int Add1(int[] dest, int[] x, int size, int y) {
		 ...
		 */
		public static int AddN(int[] dest, int[] x, int[] y, int len) {
			long carry = 0;
			for (int i = 0; i < len; i++) {
		 ...
		 */
		public static int SubtractN(int[] dest, int[] X, int[] Y, int size) {
			int cy = 0;
			for (int i = 0; i < size; i++) {
				int y = Y[i];
				int x = X[i];
		...
		*/
		public static int Multiply1(int[] dest, int[] x, int len, int y) {
		...
		public static void Multiply(int[] dest, int[] x, int xlen, int[] y, int ylen) {
			dest[xlen] = MPN.Multiply1(dest, x, xlen, y[0]);
 ...
 */
		public static long udiv_qrnnd(long N, int D) {
			long q, r;
			long a1 = ByteBuffer.URShift(N, 32);
			long a0 = N & 0xffffffffL;
			if (D >= 0) {
				if (a1 < ((D - a1 - ByteBuffer.URShift(a0, 31)) & 0xffffffffL)) {
				...
				long b1 = ByteBuffer.URShift(D, 1);	/* d/2, between 2^30 and 2^31 - 1 */
				//long c1 = (a1 >> 1); /* A/2 */
				//int c0 = (a1 << 31) + (a0 >> 1);
				long c = ByteBuffer.URShift(N, 1);
				if (a1 < b1 || (a1 >> 1) < b1) {
					if (a1 < b1) {
						q = c / b1;
						r = c % b1;
		 ...
		 */
		public static int DivideMod1(int[] quotient, int[] dividend, int len, int divisor) {
			int i = len - 1;
			long r = dividend[i];
		 ...
		 */
		public static int SubMultiply1(int[] dest, int offset, int[] x, int len, int y) {
		...
		public static void Divide(int[] zds, int nx, int[] y, int ny) {
			...
			do {                          // loop over digits of quotient
				...
				if (qhat != 0) {
					int borrow = SubMultiply1(zds, j - ny, y, ny, qhat);
					int save = zds[j];
		...
		public static int GetCharsPerWord(int radix) {
			if (radix < 10) {
				if (radix < 8) {
					if (radix <= 2)
						return 32;
		...
		public static int SetString(int[] dest, byte[] str, int str_len, int baseValue) {
			int size = 0;
			if ((baseValue & (baseValue - 1)) == 0) {
				// The base is a power of 2.  Read the input string from
				// least to most significant character/digit.  */

				int next_bitpos = 0;
				int bits_per_indigit = 0;
				for (int i = baseValue; (i >>= 1) != 0; ) bits_per_indigit++;
				int res_digit = 0;
				...
				int indigits_per_limb = MPN.GetCharsPerWord(baseValue);
				int str_pos = 0;
				...
				while (str_pos < str_len) {
					...
					int big_base = baseValue;

					while (--chunk > 0) {
						res_digit = res_digit * baseValue + str[str_pos++];
						big_base *= baseValue;
					}
					...
					if (size == 0)
						...
						cy_limb = MPN.Multiply1(dest, dest, size, big_base);
						cy_limb += MPN.Add1(dest, dest, size, res_digit);
					}
					if (cy_limb != 0)
						dest[size++] = cy_limb;
				}
		 ...
		 */
		public static int Compare(int[] x, int[] y, int size) {
			while (--size >= 0) {
				int x_word = x[size];
				int y_word = y[size];
				if (x_word != y_word) {
		 ...
		 */
		public static int Compare(int[] x, int xlen, int[] y, int ylen) {
			return xlen > ylen ? 1 : xlen < ylen ? -1 : Compare(x, y, xlen);
		}
		...
		public static int RShift(int[] dest, int[] x, int x_start, int len, int count) {
			...
			for (; i < len; i++) {
				...
				dest[i - 1] = (ByteBuffer.URShift(low_word, count)) | (high_word << count_2);
				low_word = high_word;
			}
			dest[i - 1] = ByteBuffer.URShift(low_word, count);
			return retval;
		}
		 ...
		 */
		public static long RShiftLong(int[] x, int len, int count) {
			int wordno = count >> 5;
			count &= 31;
			...
			if (count != 0) {
				...
				w0 = (ByteBuffer.URShift(w0, count)) | (w1 << (32 - count));
				w1 = (ByteBuffer.URShift(w1, count)) | (w2 << (32 - count));
			}
		...
		public static int LShift(int[] dest, int d_offset, int[] x, int len, int count) {
			...
			int i = len - 1;
			int high_word = x[i];
			int retval = ByteBuffer.URShift(high_word, count_2);
			d_offset++;
			while (--i >= 0) {
				int low_word = x[i];
				dest[d_offset + i] = (high_word << count) | (ByteBuffer.URShift(low_word, count_2));
				high_word = low_word;
			}
		...
		public static int GetIntLength(int i) {
			return 32 - CountLeadingZeros(i < 0 ? ~i : i);
		}

		/** Calcaulte the Common Lisp "integer-length" function.
		 * Assumes input is canonicalized:  len==BigInteger.WordsNeeded(words,len) */
		public static int GetIntLength(int[] words, int len) {
			len--;
			return GetIntLength(words[len]) + 32 * len;
		}
	}
}